...
export const AccountsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
            کارت‌ها و حساب‌های مالی
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            مجموع موجودی نقد: {formatCurrency(totalBalance, currency)} ({toPersianDigits(accounts.length)} حساب)
          </p>
        </div>
      ...
      <CollapsibleSection
        storageKey="accounts_grid"
        title="کارت‌های بانکی و کیف‌پول‌ها"
        subtitle={`${toPersianDigits(accounts.length)} حساب و کارت فعال با امکان کپی شماره کارت`}
